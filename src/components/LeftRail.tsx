    ...
    <nav style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', padding: '0 18px' }}>
        {items.map(item => {
          ...
          const content = (
            <span style={{ display: 'flex', alignItems: 'center', gap: '9px', opacity: active ? 1 : 0.42, transition: 'opacity .3s' }}>
              <span style={{
                display: 'block',
                height: '1.5px',
                width: active ? '26px' : '14px',
                background: active ? 'var(--accent)' : (item.isBack ? 'var(--text-4)' : 'var(--text-1)'),
          ...
          if (item.onClick) {
            return (
              <a
                ...
                onMouseEnter={e => { if (!active) (e.currentTarget as HTMLElement).querySelector('span')!.style.opacity = '1'; }}
                onMouseLeave={e => { if (!active) (e.currentTarget as HTMLElement).querySelector('span')!.style.opacity = '0.42'; }}
              >
                {content}
              </a>
            );
      ...
      <div style={{
        ...
        <div>SAT · TX</div>
        {openToWork && (
          <div style={{ marginTop: '3px', display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-3)' }}>
            <span style={{
              width: '7px',
              height: '7px',
              borderRadius: '50%',
